...
def get_nterminal_softclip(cigarstring):
    ...
    """
    five_clipping = re.search('^(\d+)(?=S)', cigarstring)
...
def filter_reads(df, ignore_hits=[], phix_filter=True, remove_indels=True, bits_not_allowed=[4, 512], expected_start={}, expected_end={}, debug_file=None):
    ...
    df[['nterminal_clip', 'cterminal_clip']] = df.cigar.str.extract('^(\d+(?=S))?.+?(\d+)?S?$', expand=True).fillna(0).astype(int).values  # = df['cigar'].str.extract().apply(get_nterminal_softclip)
