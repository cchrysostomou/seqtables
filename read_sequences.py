...
import gc
import warnings
from .insilica_sequences import generate_sequence, generate_library, add_quality_scores
...
def scratch_data(num_seqs, seq_len, ss_pos=[], error_prone_rate=0.01):
    """
    ...
    return seqtable(lib, qual, seq_type='NT')
    ...
    if use_pandas:
        ...
        if ignore_quotes:
            if chunk_size is not None:
                dfs = pd.read_csv(input_file, sep='\n', nrows=limit * 4, chunksize=chunk_size, quotechar='\x07', header=None)
            else:
                dfs = [pd.read_csv(input_file, sep='\n', nrows=limit * 4, quotechar='\x07', header=None)]
        else:
            if chunk_size is not None:
                dfs = pd.read_csv(input_file, sep='\n', nrows=limit * 4, chunksize=chunk_size, header=None)
            else:
                dfs = [pd.read_csv(input_file, sep='\n', nrows=limit * 4, header=None)]
        ...
        for seq in SeqIO.parse(input_file, 'fastq'):
            ...
            seqs.append(r[1])
            quals.append(r[3])
            if limit is not None and l > limit * 4:
                break
